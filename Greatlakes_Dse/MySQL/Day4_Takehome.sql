...
select ba.Account_Number,sum(distinct(bt.Transaction_amount))as other_transactions,
sum(distinct(bt2.Transaction_amount)) as friday_transaction
from bank_account_details ba
join bank_account_transaction bt
on ba.Account_Number=bt.Account_Number and Account_type='CREDIT CARD' 
and date_format(bt.TRANSACTION_date,'%W')<>'FRIDAY'
left join bank_account_transaction bt2
on ba.Account_Number=bt2.Account_Number and Account_type='CREDIT CARD' 
and date_format(bt2.TRANSACTION_date,'%W')='FRIDAY'
group by ba.Account_Number;



# Question 2:
# 2) Show the details of credit cards whose aggregate transaction amount during holidays  
# is greater than other days .   

select sum(distinct(bt.transaction_amount)),sum(distinct(bt2.Transaction_amount))
from bank_account_details bd join bank_account_transaction bt on bt.Account_Number=bd.Account_Number
and bd.Account_type='credit card' and bt.transaction_date in (select holiday from bank_holidays)
left join bank_account_details bd2 on bd2.Account_Number=bd.Account_Number join bank_account_transaction bt2 on bt2.Account_Number=bd2.Account_Number
and bd2.Account_type='credit card' and bt2.Transaction_Date not in (select holiday from bank_holidays)
group by bd.account_number;
 

# Question 3:
# 3) Generate a report to Send Ad-hoc holiday greetings - “Happy Holiday” for all 
# transactions occurred during Holidays in 4th month.

select bt.Account_Number,Customer_message
from bank_account_transaction bt join bank_holidays bh on bt.transaction_date=bh.holiday
and month(bh.holiday)=3 cross join bank_customer_messages where event='adhoc' and customer_message='Happy holiday' ;

# No records found for value 4th month 

select bt.Account_Number,Customer_message
from bank_account_transaction bt join bank_holidays bh on bt.transaction_date=bh.holiday
and month(bh.holiday)=4 cross join bank_customer_messages where event='adhoc' and customer_message='Happy holiday' ;
...
select bd.account_number,datediff(curdate(),transaction_date), Transaction_amount*interest_rate as accrued_interest
from bank_account_details bd join bank_account_transaction bt on bt.Account_Number=bd.Account_Number
and Account_type= 'recurring deposits' and datediff(curdate(),transaction_date)>30 join bank_interest_rate bi
on bd.account_type=bi.account_type ;
...
select br3.account_number
from bank_account_relationship_details br1 join bank_account_relationship_details br2 
on br1.linking_account_number=br2.account_number
and br1.account_type like '%credit%' join bank_account_transaction bt 
on bt.account_number=br1.account_number
join bank_account_relationship_details br3 on br3.account_number=br1.linking_account_number
group by bt.account_number having count(bt.account_number)>1 ;

# Question 6:
# 6) Display the Savings Account number whose corresponding AddonCredit card transactions have occured atleast once. 

select br3.linking_account_number
from bank_account_relationship_details br2 join bank_account_relationship_details br1 
on br2.linking_account_number=br1.account_number
join bank_account_transaction bt on br2.account_number=bt.account_number and 
br2.account_type like '%Add%' join bank_account_relationship_details br3 
on br2.linking_account_number=br3.account_number
group by br2.account_number;
...
select bc.customer_id,bc.customer_name,bc.address from bank_customer bc join  bank_customer_export be using (customer_id);
