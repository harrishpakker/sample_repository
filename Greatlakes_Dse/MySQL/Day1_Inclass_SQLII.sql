...
select orderNumber from orders where orderNumber in (select orderNumber from orderdetails group by orderNumber
 having sum(quantityOrdered*priceEach) > 100000) and status like 
"c%";
...
(select customernumber from orders where datediff(orderdate,shippeddate) <= 2));

#Q12. Get product name , product line , product vendor of products that got cancelled(53 Rows)
select productname , productline , productvendor from products where productCode in ( select distinct(productCode) 
from orderdetails where ordernumber in (select ordernumber from orders where status = 'Cancelled')); 
 
#Q13. Get customer full name along with phone number ,address , state, country, who's order was resolved(4 Rows)
select customerNumber,phone,addressline1,state,country
from customers where customerNumber in (
select customernumber from orders where status like "r%");	
